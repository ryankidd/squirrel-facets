	...
	public function get_query_facets( WP_Query $query, ?array $facets = null ) : array {
		...
		foreach ( $facets as $facet ) {
			...
			foreach ( $ep_facet_aggs[ $facet ] as $value => $count ) {
				...
				if ( 'post_type' === $type && post_type_exists( $value ) ) {
					$result->label = get_post_type_labels( get_post_type_object( $value ) )->name ?? $result->label;
				} elseif ( 'taxonomy' === $type && ! empty( $taxonomy_terms[ $value ] ) ) {
					$result->label = $taxonomy_terms[ $value ]->name ?? $result->label;
				} elseif ( 'meta' === $type ) {
					$result->label = $this->humanize( $result->label );
				} elseif ( 'post_author' === $type ) {
					$author        = get_user_by( 'slug', $value );
					$result->label = $author->display_name ?? $result->label;
				}
