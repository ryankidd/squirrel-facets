...
use Squirrel\Facets\Configurable;
use Squirrel\Facets\Facet;
use Squirrel\Facets\FacetSet;
...
abstract class AbstractProvider extends Configurable implements ProviderInterface {
	...
	public function get_other_taxonomy_filters( Facet $facet ) : array {
		...
		if ( ! empty( $values ) ) {
			...
			$sql .= $wpdb->prepare( "AND t.slug NOT IN ($placeholders)", $values ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		}

		$missing_term_ids = array_map( 'intval', $wpdb->get_col( $sql ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	...
	public function init() : void {
		 ...
		 * @return string
		 */
		$this->query_var = (string) apply_filters( 'squirrel/facets/query_var', $this->query_var );
	}
	...
	public function pre_get_posts( $query ) : void {
		...
		if ( empty( $facets ) ) {
			 ...
			 * @return bool
			 */
			$can_facet = (bool) apply_filters( 'squirrel/facets/auto', $can_facet, $query );
		 ...
		 * @return array
		 */
		$facets = array_filter( (array) apply_filters( 'squirrel/facets/facets', $facets, $query ) );
	...
	public function the_posts( $posts, $query ) {
		...
		if ( ! empty( $facets ) ) {
			...
			if ( ! empty( $results ) ) {
				...
				foreach ( $query_facets as $facet ) {
					$query->facets[ $facet ] = new Facet(
						[
							'name'      => $facet,
				...
				foreach ( $query->facets as $facet ) {
					if ( $facet->hierarchical ) {
						...
						if ( ! empty( $filters[ $facet->name ] ) ) {
							$facet->mark_active_parents( $filters[ $facet->name ] );
						}
					}
	...
	public function redirect_canonical( $redirect_url, $requested_url ) {
		if ( $redirect_url !== $requested_url ) {
			$url_query = parse_url( $requested_url, PHP_URL_QUERY ); // phpcs:ignore WordPress.WP.AlternativeFunctions

			if ( ! empty( $url_query ) ) {
				parse_str( $url_query, $query_string ); // phpcs:ignore WordPress.WP.AlternativeFunctions
