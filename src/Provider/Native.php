...
namespace Squirrel\Facets\Provider;

use Squirrel\Facets\Native\Index;
use Squirrel\Facets\Native\Settings;
use Squirrel\Facets\Native\Command;
use Squirrel\Facets\Native\Post;
use Squirrel\Facets\Native\RESTController;
use Squirrel\Facets\Source\SourceInterface;
use WP_Query;
use WP_Term;
use WP_CLI;
...
class Native extends AbstractProvider implements ProviderInterface {
	 ...
	 * @return void
	 */
	public function cli_init() : void {
		WP_CLI::add_command( 'squirrel facets', new Command( $this ) );
	}
	...
	public function get_query_facets( WP_Query $query, ?array $facets = null ) : array {
		...
		$sub_query = "SELECT $wpdb->posts.ID FROM $wpdb->posts $join WHERE $where";
		 ...
		 * @return string
		 */
		$sub_query = apply_filters( 'squirrel/facets/sub_query', $sub_query, $query );
		...
		$cacheable   = ! str_contains( strtoupper( $query->clauses['orderby'] ?? '' ), 'RAND(' );
		$cache_group = 'squirrel-facets-results';
		$cache_key   = hash( 'md5', $sql ) . ':' . wp_cache_get_last_changed( 'posts' ) . ':' . wp_cache_get_last_changed( 'post-queries' );

		if ( ! empty( $query->tax_query->queries ) ) {
			$cache_key .= ':' . wp_cache_get_last_changed( 'terms' );
		}
		...
		if ( ! $cache_found || ! is_array( $results ) ) {
	...
	public function can_facet_post_type() : bool {
		return (bool) apply_filters( 'squirrel/facets/post_type', true );
	}
	...
	public function can_facet_author() : bool {
		return (bool) apply_filters( 'squirrel/facets/author', false );
	}
	...
	public function get_taxonomies() : array {
		$taxonomies = (array) apply_filters( 'squirrel/facets/taxonomies', array_filter( (array) get_option( 'facets_taxonomies', [] ) ) );
	...
	public function get_meta_keys() : array {
		return (array) apply_filters( 'squirrel/facets/meta_keys', array_filter( (array) get_option( 'facets_meta_keys', [] ) ) );
	}
