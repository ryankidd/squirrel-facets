...
class Facet extends Configurable implements Iterator {
	...
	public function show_all( bool $show_empty_tree = false, array $active = [] ) {
		...
		if ( ! empty( $missing ) ) {
			...
			if ( $this->hierarchical ) {
				$this->build_tree( $show_empty_tree, $active );
			}
	 ...
	 * @param array $active - explicitly active filter slugs
	 * @return static
	 */
	public function build_tree( $show_empty_branches = false, array $active = [] ) : array {
		...
		if ( ! empty( $active ) ) {
			$this->mark_active_parents( $active );
		}
	...
	/**
	 * Mark parent facets as active when the facet is hierarchical and the parent facet is selected but the child facet matches
	 *
	 * @param array $active - the list of facets explicitly selected
	 * @return void
	 */
	public function mark_active_parents( array $active ) : void {
		// this marks the parent facet as active when the result only matches a child facet value but the parent was explicitly selected
		$mark = function ( $facet_filter ) use ( &$mark, $active ) {
			if ( in_array( $facet_filter->value, $active, true ) ) {
				$facet_filter->active = true;
			}

			if ( ! empty( $facet_filter->children ) ) {
				array_walk( $facet_filter->children, $mark );
			}
		};

		array_walk( $this->filters, $mark );
	}
	 ...
	 * @return void
	 */
	public function sort( ?string $orderby = null, ?string $order = null ) : void {
