...
namespace Squirrel\Facets\Native;
...
class Settings {
	 ...
	 * @var string
	 */
	protected string $settings_page = 'squirrel-facets';
		...
		if ( ! $this->index->exists() ) {
			add_settings_error(
				...
				sprintf(
					...
					add_query_arg(
						[
							'action'   => 'squirrel_facets_index',
							'_wpnonce' => wp_create_nonce( 'squirrel_facets_index' ),
						],
	...
	public function prepare_facets_taxonomies( $args ) {
		...
		$args['options'] = array_combine(
			array_column( $taxonomies, 'name' ),
			array_map( fn( $taxonomy ) => sprintf( '%s (%s)', $taxonomy->label, $taxonomy->name ), $taxonomies )
		);
